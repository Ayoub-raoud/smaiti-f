...
export default function AdminClients() {
  ...
  // Helper function to get car from reservation
  ...
  // Helper function to get matricule from reservation
  ...
  // ---- Helper to safely get payment_history array ----
  ...
  return (
    <>
      {/* Client Details - Full Page Inline */}
      {showClientDetails && selectedClient && (
        <div className="inline-details-container">
          <div className="inline-details-header">
            ...
            <button onClick={() => setShowClientDetails(false)} className="inline-details-close">
              <X size={24} />
            </button>
          </div>
            ...
            <div className="client-profile-section">
              ...
              <div className="client-identity-grid">
                <div className="identity-item">
                  ...
                  <span className="identity-value">{selectedClient.cin_number || "—"}</span>
                </div>
                <div className="identity-item">
                  <span className="identity-label">Permis</span>
                  <span className="identity-value">{selectedClient.driver_license_number || "—"}</span>
                </div>
                <div className="identity-item">
                  <span className="identity-label">Date naissance</span>
                  <span className="identity-value">{formatDate(selectedClient.date_naissance)}</span>
                </div>
                <div className="identity-item">
                  <span className="identity-label">Lieu naissance</span>
                  <span className="identity-value">{selectedClient.lieu_naissance || "—"}</span>
                </div>
                <div className="identity-item">
                  <span className="identity-label">CIN délivré le</span>
                  <span className="identity-value">{formatDate(selectedClient.cin_delivre_le)}</span>
                </div>
                <div className="identity-item">
                  <span className="identity-label">Permis délivré le</span>
                  <span className="identity-value">{formatDate(selectedClient.permis_delivre_le)}</span>
                </div>
              </div>
              ...
              return (
                <div className="client-stats-section">
                  <div className="stats-grid-5">
                    ...
                    <div className="stat-item-details">
                      <div className="stat-value text-warning">{stats.activeReservations}</div>
                      <div className="stat-label">Réservations actives</div>
                    </div>
                    <div className="stat-item-details">
                      <div className="stat-value text-success">{stats.completedReservations}</div>
                      <div className="stat-label">Réservations terminées</div>
                    </div>
                    <div className="stat-item-details">
                      <div className="stat-value text-danger">{stats.accidentCount}</div>
                    ...
                    <div className="stat-item-details">
                      <div className="stat-value text-success">{formatCurrency(stats.totalPaid)}</div>
                      <div className="stat-label">Total payé</div>
                    </div>
                    <div className="stat-item-details">
                      <div className="stat-value text-danger">{formatCurrency(stats.totalRemaining)}</div>
                        ...
                        return (
                          <div key={res.id} className="reservation-card">
                            <div className="reservation-card-header">
                              <div className="reservation-id">Réservation #{res.id}</div>
                            ...
                            <div className="reservation-card-body">
                              <div className="reservation-vehicle">
                                <Car size={20} />
                                <div>
                                  <div className="vehicle-name">{car ? `${car.brand} ${car.model}` : '—'}</div>
                                  <div className="vehicle-matricule">{matricule?.matricule_code || '—'}</div>
                                </div>
                              </div>
                              <div className="reservation-dates">
                                <Calendar size={16} />
                                <div>
                                  <div>Du {formatDate(res.start_date)} à {res.start_time || "08:00"}</div>
                                  <div>Au {formatDate(res.end_date)} à {res.end_time || "18:00"}</div>
                              ...
                              <div className="reservation-amounts">
                                <DollarSign size={16} />
                                <div>
                                  <div>Total: <strong>{formatCurrency(res.total_price)}</strong></div>
                                  <div>Payé: {formatCurrency(res.amount_paid)}</div>
                                  <div className={res.remaining_amount > 0 ? 'text-danger' : 'text-success'}>Reste: {formatCurrency(res.remaining_amount)}</div>
                                </div>
                              </div>
                            </div>
                          </div>
                        ...
                        return (
                          <div key={acc.id} className="accident-card">
                            <div className="accident-card-header">
                              <div className="accident-id">Accident #{acc.id}</div>
                              <div>{getAccidentStatusBadge(acc.status)}</div>
                            </div>
                            <div className="accident-card-body">
                              ...
                              <div className="accident-vehicle">
                                ...
                                <span className="matricule">{matricule?.matricule_code || '—'}</span>
                              </div>
                              <div className="accident-type">
                                <span className={`badge ${acc.accident_type === 'grave' ? 'badge-danger' : 'badge-warning'}`}>
                                  {acc.accident_type === 'grave' ? 'Accident grave' : 'Accident non grave'}
                                </span>
                              </div>
                              <div className="accident-amounts">
                                <div>Pertes: {formatCurrency(acc.amount_of_losses)}</div>
                                <div>Assurance: {formatCurrency(acc.amount_assurance)}</div>
                              </div>
                              {acc.description && (
                            ...
                            <div key={res.id} className="payment-card-full">
                              <div className="payment-card-header-full">
                                ...
                                <div className="payment-amounts-full">
                                  <span>Total: {formatCurrency(res.total_price)}</span>
                                  <span className="paid-amount">Payé: {formatCurrency(res.amount_paid)}</span>
                                  <span className={`remaining-amount ${(res.remaining_amount || 0) > 0 ? 'negative' : 'positive'}`}>
          ...
          <div className="inline-details-footer">
            <button onClick={() => setShowClientDetails(false)} className="btn-secondary-full">
              Fermer
            </button>
          </div>
        </div>
      ...
      {/* Client Form - Full Page Inline */}
      {showClientForm && (
        <div className="inline-form-container">
          <div className="inline-form-header">
            <div className="inline-form-icon">
              {editingClient ? <Sparkles size={28} /> : <Star size={28} />}
            </div>
            ...
            <button onClick={() => { setShowClientForm(false); setEditingClient(null); resetForm(); setCinPreview(''); setLicensePreview(''); }} className="inline-form-close">
              <X size={24} />
            </button>
          </div>
          ...
          <form onSubmit={(e) => {
              ...
              <div className="inline-form-col">
                <div className="inline-section">
                  ...
                  <div className="inline-info-message">
                    <Info size={16} />
                    <span>Les documents sont optionnels mais recommandés pour les dossiers clients</span>
                  </div>
                </div>
                    ...
                    <div className="inline-info-grid">
                      <div className="inline-info-item">
                        ...
                        <span className="info-value">
                          {editingClient.created_at ? formatDate(editingClient.created_at) : "—"}
                        </span>
                      </div>
                      <div className="inline-info-item">
                        <span className="info-label">Dernière modification</span>
                        <span className="info-value">
                          {editingClient.updated_at ? formatDate(editingClient.updated_at) : "—"}
                        </span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="inline-form-footer">
              <button type="button" className="inline-secondary-btn" onClick={() => { setShowClientForm(false); setEditingClient(null); resetForm(); setCinPreview(''); setLicensePreview(''); }}>
                Annuler
              </button>
              <button type="submit" className="inline-primary-btn" disabled={submitting}>
                {submitting ? "Traitement..." : (editingClient ? "Mettre à jour" : "Créer le client")}
              </button>
            </div>
          </form>
        </div>
        ...
        <div className="admin-container">
          <div className="header">
            <div>
              ...
              <p className="subtitle">Base de données clients et documents</p>
            </div>
          ...
          <div className="stats-grid">
            ...
            <div className="stat-card">
              ...
              <IdCard size={32} className="stat-icon text-green" />
            </div>
            <div className="stat-card">
              <div>
              ...
              <FileText size={32} className="stat-icon text-emerald" />
            </div>
            <div className="stat-card">
              <div>
              ...
              <Mail size={32} className="stat-icon text-blue" />
            </div>
          </div>
          ...
          <div className="table-info">
            <p className="table-info-text">{filteredClients.length} client(s) trouvé(s)</p>
                      ...
                      <td>
                        ...
                        <td className="text-right">
                          <div className="action-buttons">
                            <button onClick={() => openDetails(c)} className="action-btn action-btn-view" title="Détails">
                              <Eye size={16} />
                            </button>
                            <button onClick={() => handleEdit(c)} className="action-btn action-btn-edit" title="Modifier">
                              <Edit2 size={16} />
                            </button>
              ...
              <div className="modal-overlay">
                <div className="modal delete-modal">
                  ...
                  <p className="delete-message">
                    ...
                    <span className="client-name">{clientToDelete.prenom} {clientToDelete.nom}</span> ?<br />
                    Cette action est irréversible.
                  </p>
                  {getClientReservations(clientToDelete.id).length > 0 && (
                    <p className="delete-message" style={{ fontSize: "0.75rem", color: "#dc2626" }}>
                      ⚠️ Ce client a {getClientReservations(clientToDelete.id).length} réservation(s) associée(s)
                    </p>
                  )}
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* Inline Details Container (Full Page Details) */
  ...
  .details-tab-full:hover {
    background: #f1f5f9;
  }

  .details-tab-full.active {
    background: #0f172a;
    color: white;
  }
  ...
  .accident-date, .accident-vehicle {
    display: flex;
    align-items: center;
    gap: 8px;
    font-size: 0.875rem;
  ...
  @media (prefers-color-scheme: dark) {
    ...
    .btn-secondary:hover, .btn-secondary-full:hover { background: #475569; }
    .search-input, .inline-input, .inline-select {
      background: #0f172a;
      border-color: #334155;
      color: #f1f5f9;
    }
    .inline-section, .client-profile-section, .client-stats-section {
      background: #0f172a;
      border-color: #334155;
    }
    .inline-section-header h3, .inline-info-item .info-value, .client-name,
    .client-profile-info h3, .identity-value, .reservation-id, .accident-id,
    .vehicle-name {
      color: #f1f5f9;
    }
    .inline-info-message {
    ...
    .details-tab-full:hover:not(.active) {
      background: #334155;
    }
    .sortable-header:hover {
