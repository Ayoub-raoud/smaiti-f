  ...
  const filteredReservations = useMemo(() => {
    ...
    if (filterParam === 'notifications') {
      filtered = filtered.filter(r => {
        ...
        return isLate || isEndingSoon;
      });
    } else if (filterParam === 'pending') {
      filtered = filtered.filter(r => r.status === 'pending');
    } else if (filterParam === 'contacted') {
      filtered = filtered.filter(r => r.status === 'contacted');
    } else if (filterParam === 'retard') {
      filtered = filtered.filter(r => r.status === 'retard');
    } else if (filterParam === 'ending-soon') {
      filtered = filtered.filter(r => {
        ...
        <div className="admin-container">
          <div className="header">
            <div>
              ...
              <p className="subtitle">{filteredReservations.length} réservation(s) (hors en attente, annulé, contacté)</p>
            </div>
          ...
          <div className="search-wrapper">
            ...
            {filterParam === 'notifications' && (
              <div className="notification-filter-banner" style={{ marginTop: '12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fef3c7', padding: '0.75rem 1rem', borderRadius: '8px', border: '1px solid #f59e0b' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>🔔 Affichage des notifications uniquement</span>
                <button onClick={() => setSearchParams({})} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#92400e', fontWeight: '500' }}><X size={14} /> Effacer le filtre</button>
              </div>
            )}
          </div>
